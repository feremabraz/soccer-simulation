...
export class OffsideRule {
  ...
  isPlayerOffside(
    ...
    const direction = isHome ? 1 : -1 // Direction of attack
  ...
  calculateOffsideLine(players: PlayerEntity[]): { home: number; away: number } {
    ...
    const homeLine = homeDefendersPositionsX.length >= 2 ? 
      homeDefendersPositionsX[1] : 
      40
    
    const awayLine = awayDefendersPositionsX.length >= 2 ? 
      awayDefendersPositionsX[1] : 
      60
