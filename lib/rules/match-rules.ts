...
import type { EventType, TeamSide, Vector2D } from '../utils/types';
...
export class MatchRules {
  ...
  detectFoul(
    ...
    const isPenalty = this.isInPenaltyArea(
      tackledPlayer.position,
      tackledPlayer.team === 'home' ? 'away' : 'home'
    );
  ...
  calculateStoppageTime(regularTime: number, events: { type: EventType; time: number }[]): number {
    ...
    // Cards add time
    const cards = eventsInThisHalf.filter(
      (e) => e.type === 'yellowcard' || e.type === 'redcard'
    ).length;
